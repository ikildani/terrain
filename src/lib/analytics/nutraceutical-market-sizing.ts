...
const HEALTH_FOCUS_TO_COMPETITIVE_CATEGORY: [RegExp, string][] = [
  ...
  // Sports / performance
  [
    /muscle|athletic|performance|recovery|endurance|power output|exercise|creatine|testosterone.*sport/i,
    'sports_nutrition',
  ],
  // Collagen / beauty
...
const NUTRA_STAGE_SHARE: Record<string, { low: number; base: number; high: number }> = {
  ...
  market_ready: { low: 0.003, base: 0.008, high: 0.02 },
  commercial: { low: 0.008, base: 0.02, high: 0.06 },
};
...
const NUTRA_REVENUE_RAMP_PROFILES: Record<string, number[]> = {
  ...
  novel_fast: [0.15, 0.35, 0.6, 0.8, 0.92, 1.0, 0.98, 0.94, 0.88, 0.82],
  // Mature / crowded: slower ramp into saturated market, long plateau, gradual decline
  mature_sustained: [0.06, 0.15, 0.3, 0.5, 0.68, 0.82, 0.92, 1.0, 0.98, 0.95],
  // Clinical-driven: slow start (evidence must build), strong mid-phase as data publishes, long tail
  clinical_driven: [0.05, 0.12, 0.28, 0.5, 0.72, 0.88, 0.95, 1.0, 1.0, 0.97],
  // DTC viral: explosive start, peaks early, decays as novelty fades
  dtc_viral: [0.2, 0.45, 0.7, 0.9, 1.0, 0.95, 0.88, 0.8, 0.72, 0.65],
  // Default: balanced (original ramp — used when no category match)
  default: [0.1, 0.25, 0.5, 0.72, 0.88, 0.95, 1.0, 1.0, 0.97, 0.93],
};
...
function selectRevenueRampProfile(
  ...
  if (
    ['gut_microbiome', 'multivitamin', 'immune_foundational', 'joint_mobility', 'collagen_beauty'].includes(
      categorySlug ?? '',
    )
  ) {
...
const CLAIM_TYPE_REACH: Record<string, number> = {
  structure_function: 0.8,
  qualified_health: 0.9,
...
const NUTRA_COMPARABLE_ACQUISITIONS = [
  ...
  {
    target: "Nature's Bounty / Bountiful Company",
    acquirer: 'KKR',
    value_m: 5750,
    multiple: '~3x revenue',
    year: 2020,
...
function findIngredient(name: string): NutraceuticalIngredient | null {
  ...
  const exact = NUTRACEUTICAL_INGREDIENTS.find((i) => i.name.toLowerCase() === lower);
  ...
  for (const ingredient of NUTRACEUTICAL_INGREDIENTS) {
    const ingredientWords = ingredient.name
      ...
      .split(/[\s\-\/()]+/)
      .filter((w) => w.length > 2);
    const overlap = queryWords.filter((w) => ingredientWords.some((iw) => iw.includes(w) || w.includes(iw))).length;
...
function getHealthCategoryPenetration(healthFocus: string): {
  ...
  const match = HEALTH_FOCUS_SUGGESTIONS.find((h) => {
    ...
    return lower.includes(hLower) || hLower.includes(lower) || h.category.toLowerCase().includes(lower);
  });

  // Try to match to US_SUPPLEMENT_MARKET.top_categories
  const categoryMatch = US_SUPPLEMENT_MARKET.top_categories.find(
    (c) => lower.includes(c.category.toLowerCase()) || c.category.toLowerCase().includes(lower),
  );
...
function getDemographicFactor(target: string): number {
  ...
  if (/adult.?30.?60|30\+/.test(lower)) return 0.4;
  if (/pregnan|prenatal|fertility|maternal/.test(lower)) return 0.12;
  if (/child|kid|pediatric/.test(lower)) return 0.1;
  if (/everyone|general|all adult/.test(lower)) return 0.5;
...
function buildDataSources(
  ...
  const sources: { name: string; type: 'public' | 'proprietary' | 'licensed'; url?: string }[] = [
    ...
    {
      name: 'FDA CFSAN — Dietary Supplement Ingredient Directory',
      type: 'public',
      url: 'https://www.fda.gov/food/dietary-supplements',
    },
    {
      name: 'FTC — Health Products Enforcement Actions',
      type: 'public',
...
export async function calculateNutraceuticalMarketSizing(
  rawInput: NutraceuticalMarketSizingInput,
): Promise<NutraceuticalMarketSizingOutput> {
  // Normalize input — API schema marks many fields optional,
  // but the engine expects them. Apply safe defaults.
  const input: NutraceuticalMarketSizingInput = {
    ...rawInput,
    primary_ingredient: rawInput.primary_ingredient ?? 'Unknown Ingredient',
    health_focus: rawInput.health_focus ?? 'General Wellness',
    target_demographic: rawInput.target_demographic ?? 'Adults 30-65',
    claim_type: rawInput.claim_type ?? ('structure_function' as NutraceuticalMarketSizingInput['claim_type']),
    channels: rawInput.channels ?? (['dtc_ecommerce'] as NutraceuticalMarketSizingInput['channels']),
    unit_price: rawInput.unit_price ?? 39.99,
    units_per_year_per_customer: rawInput.units_per_year_per_customer ?? 12,
    cogs_pct: rawInput.cogs_pct ?? 30,
    development_stage: rawInput.development_stage ?? 'market_ready',
    has_clinical_data: rawInput.has_clinical_data ?? false,
    patent_protected: rawInput.patent_protected ?? false,
    nutraceutical_category:
      rawInput.nutraceutical_category ??
      ('dietary_supplement' as NutraceuticalMarketSizingInput['nutraceutical_category']),
    geography: rawInput.geography ?? ['US'],
    launch_year: rawInput.launch_year ?? new Date().getFullYear() + 1,
  };
  ...
  if (!ingredient) {
    ...
    ingredient =
      NUTRACEUTICAL_INGREDIENTS.find(
        (i) =>
  ...
  const consumerFunnel: NutraceuticalConsumerFunnel = {
    ...
    health_category_penetration_pct: Math.round((healthCategoryUsers / supplementUsers) * 10000) / 100,
    target_demographic_users: targetDemographicUsers,
    demographic_penetration_pct: Math.round((targetDemographicUsers / healthCategoryUsers) * 10000) / 100,
  ...
  for (const cr of channelRevenues) {
    cr.contribution_pct = totalNetRevenue > 0 ? Math.round((cr.net_revenue_m / totalNetRevenue) * 10000) / 100 : 0;
  }
  ...
  if (hasDTC) {
    ...
    const decayRate = retentionCurve[11] > 0 ? retentionCurve[11] / retentionCurve[10] : 0.98;
    let ltv36 = ltv12;
    ...
    const breakEvenSubscribers = annualContribution > 0 ? Math.round(500_000 / annualContribution) : 0;
  ...
  const usTamB = ingredient ? ingredientMarketSizeM / 1000 : (healthCategoryUsers * avgAnnualSpend) / 1e9;

  // US SAM: TAM * channel reach * claim type reach
  const claimReach = CLAIM_TYPE_REACH[input.claim_type] ?? 0.8;
  ...
  const tamConfidence: 'high' | 'medium' | 'low' = ingredient ? 'high' : ingredientUsedFallback ? 'medium' : 'low';

  const samConfidence: 'high' | 'medium' | 'low' = ingredient && input.channels.length > 0 ? 'medium' : 'low';
  ...
  const categoryCompetitors =
    dbCompetitors.length > 0 ? dbCompetitors : mappedCategory ? findCompetitorsByCategory(mappedCategory) : [];
  const topBrandsDB =
  ...
  if (input.channels.includes('practitioner')) diffVectors.push('Practitioner recommendation channel');
  ...
  if (input.channels.includes('subscription')) diffVectors.push('Subscription model drives recurring revenue');
  if (diffVectors.length === 0) diffVectors.push('Formulation innovation', 'Brand and marketing execution');
  ...
  if (input.channels.includes('retail_specialty'))
    certs.push('Non-GMO Project Verified', 'Certified Organic (if applicable)');
  ...
  if (!input.has_clinical_data) claimGaps.push('No proprietary clinical trial data — reliance on published literature');
  ...
  if (input.claim_type === 'structure_function')
    claimGaps.push(
      'Structure/function claims must be substantiated — FTC "competent and reliable scientific evidence" standard',
    );
  if (clinicalEvidenceLevel === 'preliminary' || clinicalEvidenceLevel === 'emerging')
    claimGaps.push(
      `Clinical evidence rated "${clinicalEvidenceLevel}" — additional RCTs may be needed for claim defense`,
    );
  ...
  const sensitivityDrivers: NutraceuticalSensitivityDriver[] = [
    {
      ...
      low_som_m: Math.round(baseSomM * 0.7 * 100) / 100,
      high_som_m: Math.round(baseSomM * 1.3 * 100) / 100,
    ...
    },
    {
      ...
      low_som_m: Math.round(baseSomM * 0.8 * 100) / 100,
      high_som_m: Math.round(baseSomM * 1.2 * 100) / 100,
      base_som_m: Math.round(baseSomM * 100) / 100,
      swing_pct: 20,
    },
  ];
  ...
  const studySummary =
    ingredientStudies.length > 0
      ? `Published evidence base: ${ingredientStudies.length} studies (${rctCount} RCTs, ${metaCount} meta-analyses). ${
          landmarkStudiesForIngredient.length > 0
            ? `Landmark studies: ${landmarkStudiesForIngredient
                .slice(0, 2)
                .map(
                  (s) =>
                    `${s.study_name} (${s.journal} ${s.year}, n=${s.sample_size}, ${s.result_summary.slice(0, 80)}...)`,
                )
                .join('; ')}.`
            : ''
        }`
  ...
  const BRAND_BUILD_BENCHMARKS: Record<string, { low: number; base: number; high: number; cpa_note: string }> = {
    ...
    retail_mass: {
      ...
      base: 6.0,
      high: 10.0,
      cpa_note: 'Slotting fees $25-50K/SKU/chain, trade promo 15-25% of revenue',
    },
    retail_specialty: {
      low: 1.0,
      base: 2.5,
      high: 4.0,
      cpa_note: 'In-store demos $500-1K/event, sampling $2-5/trial, education reps',
    },
    practitioner: {
      low: 0.8,
      base: 2.0,
      high: 3.5,
      cpa_note: 'KOL advisory board $50-100K, clinical education, Fullscript/Wellevate fees',
    },
    subscription: {
      low: 1.2,
      base: 3.0,
      high: 5.5,
      cpa_note: 'Recharge/Bold subscription tech, lifecycle email, loyalty program',
    },
  ...
  const dtcWeight =
    (channelWeights.get('dtc_ecommerce')?.weight ?? 0) + (channelWeights.get('subscription')?.weight ?? 0);
  ...
  const costBreakdown = channelCostNotes.length > 0 ? ` Key cost drivers: ${channelCostNotes.join('; ')}.` : '';
  ...
  if (input.channels.includes('amazon')) {
    ...
    const avgReviewsTop10 = hasAmazonData
      ? Math.round(
          amazonBrands.slice(0, 10).reduce((sum, c) => sum + (c.amazon_review_count ?? 0), 0) /
            Math.min(amazonBrands.length, 10),
        )
    ...
    const avgRatingTop10 = hasAmazonData
      ? Math.round(
          (amazonBrands.slice(0, 10).reduce((sum, c) => sum + (c.amazon_rating ?? 4.3), 0) /
            Math.min(amazonBrands.length, 10)) *
            10,
        ) / 10
      : 4.3;
    ...
    const ppcCpc =
      ingredientMarketSizeM > 1000 ? 2.2 : ingredientMarketSizeM > 500 ? 1.8 : ingredientMarketSizeM > 200 ? 1.4 : 0.95;

    const acos =
      ingredientMarketSizeM > 1000 ? 28 : ingredientMarketSizeM > 500 ? 24 : ingredientMarketSizeM > 200 ? 20 : 18;
  ...
  if (input.channels.includes('dtc_ecommerce')) valueDrivers.push('DTC channel with first-party customer data');
  ...
  const acquirerTypes: string[] = ['Strategic CPG (Nestle Health Science, Unilever, Church & Dwight)'];
  if (ingredientMarketSizeM > 300) acquirerTypes.push('Large pharma consumer health divisions (J&J, GSK, P&G)');
  ...
  const supplyRisk: 'low' | 'moderate' | 'high' =
    supplyConcentration === 'single_source' ? 'high' : supplyConcentration === 'limited' ? 'moderate' : 'low';

  const cogsVolatility: 'stable' | 'moderate' | 'volatile' =
    ingredient?.category === 'Foundational' ? 'stable' : ingredient?.trending ? 'moderate' : 'stable';

  const ingredientSupplyChain: NutraceuticalIngredientSupplyChain = {
    primary_ingredient_source: ingredient ? `${ingredient.name} — ${ingredient.category}` : input.primary_ingredient,
  ...
  const output: NutraceuticalMarketSizingOutput = {
    summary: {
      ...
      us_som: {
        ...
        unit: 'M',
        range: [Math.round(usSomLow * 10) / 10, Math.round(usSomHigh * 10) / 10],
      },
    ...
    methodology: buildMethodology(input, ingredient, ingredientUsedFallback, capturableBase, totalNetRevenue),
