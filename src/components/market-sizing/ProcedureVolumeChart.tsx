...
export default function ProcedureVolumeChart({ procedureVolume, peakSharePct }: ProcedureVolumeChartProps) {
  ...
  return (
    <div className="chart-container noise">
      ...
      <div role="img" aria-label="Procedure volume chart" style={{ width: '100%', height: 200 }}>
          ...
          <BarChart layout="vertical" data={data} margin={{ top: 10, right: 100, left: 10, bottom: 10 }}>
            ...
            <Tooltip
              content={({ active, payload }) => {
                ...
                const pctOfTotal = (d.count / procedureVolume.us_annual_procedures) * 100;
                return (
                  <div className="bg-navy-800 border border-navy-700 rounded-md px-4 py-3 text-xs shadow-elevated">
                    ...
                    <div className="text-slate-500 mt-1">{formatPercent(pctOfTotal, 1)} of total</div>
                  </div>
                );
              }}
            />
      ...
      <div className="flex items-center justify-between mt-4 pt-3 border-t border-navy-700">
        <span className="text-2xs text-slate-500">Peak share assumption: {formatPercent(peakSharePct ?? 15, 0)}</span>
        <span className="text-2xs text-slate-500">Source: {procedureVolume.source}</span>
      </div>
    </div>
  );
}
