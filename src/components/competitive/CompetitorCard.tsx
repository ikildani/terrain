...
function ScoreBar({ value, label }: { value: number; label: string }) {
  ...
  return (
    <div className="flex items-center gap-2.5">
      <span className="text-[10px] uppercase tracking-wider text-slate-500 w-16 flex-shrink-0">
        {label}
      </span>
  ...
  return (
    <div className="card hover:border-teal-500/20 transition-all duration-200">
      ...
      <div className="flex items-center justify-between pt-3 border-t border-navy-700/50">
        ...
        <span className="text-[10px] text-slate-500 flex-shrink-0 ml-2">{c.source}</span>
      </div>
    </div>
  );
}
