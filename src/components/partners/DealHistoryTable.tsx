...
export default function DealHistoryTable({ deals, compact }: DealHistoryTableProps) {
  if (deals.length === 0) {
    return <p className="text-xs text-slate-600 italic">No recent deal data available.</p>;
  }
  ...
  if (compact) {
    return (
      <ul className="space-y-1.5" role="list">
        {deals.map((deal, i) => (
          <li
            key={`${deal.licensed_to}-${deal.indication}-${i}`}
            className="flex items-center justify-between text-[11px] px-2 py-1.5 bg-navy-800/50 rounded"
          >
            <div className="flex-1 min-w-0">
              <span className="text-slate-300 truncate block">{deal.licensed_to}</span>
            ...
            <div className="text-right ml-3 shrink-0">
              <span className="font-mono text-teal-400 block">{formatDealValue(deal.upfront_usd)}</span>
              <span className="font-mono text-slate-500 text-2xs block">/ {formatDealValue(deal.total_value_usd)}</span>
            </div>
          </li>
        ))}
      </ul>
      ...
      <table className="w-full text-xs">
        <thead>
          <tr className="border-b border-navy-700">
            <th className="text-left py-2 px-2 text-2xs font-mono text-slate-500 uppercase tracking-wider">
              Counterparty
            </th>
            <th className="text-left py-2 px-2 text-2xs font-mono text-slate-500 uppercase tracking-wider">
              Indication
            ...
            <th className="text-right py-2 px-2 text-2xs font-mono text-slate-500 uppercase tracking-wider">
              Total Value
            </th>
        ...
        <tbody>
          {deals.map((deal, i) => (
            <tr
              key={`${deal.licensed_to}-${deal.indication}-${i}`}
              className="border-b border-navy-700/50 hover:bg-navy-800/30"
            >
